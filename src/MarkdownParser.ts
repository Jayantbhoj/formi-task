...
function appendText(text: string) {
    ...
    let last = currentContainer.lastChild as HTMLElement | null;

    if (!last || last.tagName !== "SPAN") {
        last = document.createElement("span");
        currentContainer.appendChild(last);
    }
...
function handleBackticks(count: number) {
    ...
    if (count >= 3) {
        if (mode === "block") { //we're already in block mode 
            //terminating backticks encountered, change mode back to text
            mode = "text";
            currentNode = null;
        } else {
            mode = "block";
    ...
    if (count === 1) {
        if (mode === "inline") {
            //terminating backtick encountered, change mode back to text
            mode = "text"; 
            currentNode = null;
        } else if (mode === "text") {
            mode = "inline";
